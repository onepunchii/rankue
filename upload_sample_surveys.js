  ...
  try {
    const response = await fetch('/api/simple-auth/surveys', {
      ...
      headers: {
        ...
        'Authorization': `Bearer ${document.cookie.split('polli_token=')[1]?.split(';')[0] || ''}`
      },
...
async function uploadAllSurveys() {
  console.log('📋 설문지 샘플 업로드 시작...');
  
  for (let i = 0; i < surveys.length; i++) {
    console.log(`📝 설문 ${i + 1} 업로드 중: ${surveys[i].survey.title}`);
    await uploadSurvey(surveys[i], i);
