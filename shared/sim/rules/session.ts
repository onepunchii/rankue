...
/**
 * 대전 시작(첫 샷) 때 방장이 자리에 없으면 주는 유예(2026-09-26 오너: "자리를 비울 때 일정 시간이 지나면 시작되고 타임아웃이 걸리게").
 * 방을 열어 두고 앱을 닫은 방장은 "상대가 들어왔어요" 푸시를 받고 돌아올 시간이 필요해 보통 유예(1분)보다 길게 준다.
 * 이 뒤로는 40초 시계가 돌고 세 번이면 실격패다. 한 번도 안 친 사람의 실격은 노쇼라 레이팅엔 반영하지 않는다(rating.ts).
 */
export const START_ABSENT_GRACE_MS = 120_000;
/**
 * 이미 시간 초과를 한 번 이상 받은 사람이 **여전히 자리에 없으면** 유예 없이 바로 40초 시계를 건다(2026-09-26 오너).
 * 한 번 봐줬는데도 안 돌아왔다 — 남은 사람을 매번 1분씩 더 기다리게 하지 않는다(실격까지 6분 남짓 → 3분 남짓).
 */
export const REPEAT_ABSENT_GRACE_MS = 0;
