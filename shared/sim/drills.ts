...
/**
 * 한국 기준 주차 id — 한국 월요일 00:00 에 바뀐다(2026-09-26 검토). weekIdFor 는 UTC 라 한국 월요일 오전 9시에 주가 바뀌어,
 * 일요일 밤 늦게 치는 사람은 9시간을 잃었고 그 경계를 넘긴 드릴은 채점에 실패했다. 이미 저장된 주차 id 는 모양이 같아 그대로 쓴다
 * (경계가 9시간 당겨질 뿐이다).
 */
export function kstWeekIdFor(ms: number): string {
    return weekIdFor(ms + 9 * 60 * 60 * 1000);
}
