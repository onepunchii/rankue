 ...
 *
 * 상단 띠의 EmojiBar 는 **건드리지 않는다**(6개 그대로) — 375 px 에서 이미 폭이 빠듯해 버튼을 더 넣으면 넘친다.
 * 문구 키는 전부 `sim.emoji.<code>` 한 갈래로 둔다(그 이름이 곧 "고정 문구"라는 뜻이다).
 */
export const CHAT_EXTRA_CODES = ["oops", "wait", "thanks"] as const;
/** 서버가 받아 주는 코드 전부(상단 띠의 여섯 + 새 셋). 넉넉히 받아 두면 화면 쪽을 바꿔도 서버를 안 건드린다. */
 ...
 * 상대가 사이에 하나 보내면 내 간격이 즉시 풀린다(repo.sendEmoji). 채팅은 **내 마지막 시각만** 보면 되는데,
