...
//   - iOS: 자동. autoWhenStoreVersion 이상이 App Store 에 실제로 올라오면(서버가 애플 공개 조회로 확인,
...
export type UpdatePolicy = {
    /** true 면 무조건 켠다. false 여도 autoWhenStoreVersion 조건이 맞으면 켜진다. */
    enabled: boolean;
    /** 스토어에 올라가 있는 버전이 이 버전 이상이면 켠 것으로 본다(iOS 전용 — 스토어 버전은 서버가 알려 준다). */
    autoWhenStoreVersion?: string;
...
export const APP_UPDATE_POLICY: Record<"ios" | "android", UpdatePolicy> = {
    ...
    ios: { enabled: false, autoWhenStoreVersion: "1.2", latestBuild: 7, minBuild: 0 },
...
export function isPolicyActive(policy: UpdatePolicy | undefined, storeVersion?: string | null): boolean {
    ...
    if (!policy.autoWhenStoreVersion || !storeVersion) return false;
    const c = compareVersions(storeVersion, policy.autoWhenStoreVersion);
    return c !== null && c >= 0;
}
