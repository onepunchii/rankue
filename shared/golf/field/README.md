...
## 화면 입력(client/src/golf/field) — 방식은 하나뿐이다
`ArcSwing.tsx`(오너 확정, 골프 슈퍼 크루 구성). 화면 전체를 덮는 투명 오버레이 — 위쪽 아크 미터, 가운데 티 위의 공 + POWER %, 아래 큰 공 + DRAW/FADE 링.
1) 링 한가운데의 공을 뒤로 끌어 **파워**(끌수록 공이 커진다, 145 px = 100 %·167 px = 115 %), 2) 끌면서 좌우로 **DRAW/FADE**(= 엔진 `stanceDeg10`, 120 px 에 ±5°), 3) 놓으면 아크를 바늘이 왕복 → 창에 올 때 **탭**(= `impactMs`, 페이스).
화면의 창은 실제 파워로 계산한다(`zoneMsFor(club, "tee", power, stance, ballPos)`) — 100 % 를 넘기면 창이 좁아지는 동시에 바늘이 빨라지고(115 % 에서 0.65배) 아크가 떨린다.
창 밖이면 당점까지 무너진다 — **빨리(왼쪽) = 대가리(`tapY` +), 늦게(오른쪽) = 뒷땅(`tapY` −)**, `tapY = ∓min(1.5, |t|−1)·45`. 4번 지나도록 안 치면 헛스윙.
