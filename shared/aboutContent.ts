...
export const ABOUT_CONTENT: Record<string, AboutContent> = {
  ...
  ja: {
    metaTitle: "RANKUE（ランキュー）とは · 手のひらのビリヤード得点板",
  ...
  // zh 는 **대만 표기(번체)** 로 통일한다. hreflang 도 zh-Hant 로 나간다(HREFLANG_OF).
  // 용어: 撞球(대만의 '당구', 중국 본토의 台球 아님) · 開崙(carom) · 三顆星(3쿠션)
  //      · 社團(동호회) · 螢幕(화면) · 帳號(계정) · 選單(메뉴) · 復原(되돌리기)
  zh: {
    metaTitle: "關於 RANKUE · 掌上撞球記分板",
    metaDesc: "RANKUE 是掌上撞球記分板。用手機輕點記分，自動計算局數、平均與最高連續得分，配對對手並保存紀錄與排名。",
    h1: "RANKUE 是什麼？",
    tagline: "掌上撞球記分板",
    intro: "RANKUE 讓手機變成撞球記分板。輕點幾下即可記分，自動計算局數、平均與最高連續得分，並有語音提示。從單人練習到與對手的配對對戰，再到戰績與排名，一應俱全。",
    whatTitle: "你可以做什麼",
    whatBody: "無需心算，用手機記分即可。局數、平均與最高連續得分自動統計，還有復原與語音提示。比賽結束後自動存入紀錄，你還能與程度相近的對手和社團一起打球。",
    featuresTitle: "核心功能",
    features: [
      { name: "掌上記分板", desc: "輕點記分。自動計算局數、平均、最高連續得分，語音提示與復原。" },
      { name: "配對對戰", desc: "與對手 1 對 1。用 PIN 碼讓對方進入同一記分板。" },
      { name: "比賽紀錄", desc: "每場比賽自動保存——戰績、平均與最高連續得分一目了然。" },
      { name: "社團", desc: "建立或加入撞球社團，分享活動、照片並聊天。" },
      { name: "模擬器", desc: "在螢幕上的虛擬球檯單人練習三顆星／四球。" },
      { name: "排名", desc: "球館與全國的三顆星、四球排行榜。" },
    ],
    whyTitle: "為什麼選 RANKUE？",
    whyBody: "把球館裡的紙和計算機換成一支手機。比賽都會保存下來，你看得見自己的進步，也能配對到合適的對手。讓打撞球更輕鬆、更有趣。",
    howTitle: "如何開始",
    how: [
      "安裝 App，用手機號碼或社群帳號登入",
      "打開記分板就能記分——單人練習或配對對戰",
    ...
    faqTitle: "常見問題",
    faq: [
      { q: "RANKUE 免費嗎？", a: "是的，免費使用，iOS 和 Android 都可以。" },
      { q: "如何記分？", a: "在螢幕上的記分板輕點輸入得分，局數、平均與最高連續得分會自動計算並語音提示，也可以復原。" },
      { q: "支援哪些項目？", a: "三顆星與四球（開崙）撞球。" },
      { q: "可以刪除帳號嗎？", a: "可以，在 App 的 選單 → 刪除帳號 中永久刪除。" },
    ],
    ctaTitle: "立即開始",
    ctaBody: "掌上撞球記分板 —— RANKUE。",
    langLabel: "語言",
    home: "首頁",
    support: "支援",
    privacy: "隱私權政策",
...
// ── hreflang 코드 매핑 ────────────────────────────────────────────────
// URL 파라미터(?lang=)와 hreflang 값이 항상 같지는 않다. zh 문안은 대만 표기(번체)로
// 쓰므로 hreflang 은 "zh" 가 아니라 "zh-Hant" 여야 한다 — "zh" 로 두면 간체권까지
// 같은 문서로 묶여 대만·홍콩 검색에서 의도한 표기가 흐려진다.
// 프리렌더(page)와 사이트맵(entry)이 같은 표를 쓴다.
export const HREFLANG_OF: Record<string, string> = { zh: "zh-Hant" };
export const hreflangOf = (lang: string) => HREFLANG_OF[lang] ?? lang;
