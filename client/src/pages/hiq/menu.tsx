    ...
    return (
        <div className="min-h-screen bg-[#0A0A0A] text-white p-6 pb-40 font-sans relative overflow-x-hidden">
            ...
            <div className="relative z-10 flex items-center justify-between mb-8 pt-5">
                <h1 className="text-[26px] font-bold tracking-tight text-white">전체 메뉴</h1>
                <div className="flex gap-2.5">
                    <button
                        title="알림"
                        onClick={() => setNotifOpen(true)}
                        className="w-11 h-11 rounded-full bg-surface-1 border border-surface-line flex items-center justify-center active:scale-95 transition-transform"
                    >
                        <LucideBell className="w-5 h-5 text-white/50" />
                    </button>
                    <button title="설정" className="w-11 h-11 rounded-full bg-surface-1 border border-surface-line flex items-center justify-center active:scale-95 transition-transform">
                        <LucideSettings className="w-5 h-5 text-white/50" />
                    </button>
                </div>
            </div>
                ...
                <div className="rk-card p-6">
                    <div className="flex items-center gap-4">
                        <div className="relative group/avatar cursor-pointer shrink-0" onClick={() => document.getElementById('profile-upload')?.click()}>
                            <div className="w-[68px] h-[68px] rounded-2xl bg-surface-2 border border-surface-line flex items-center justify-center overflow-hidden relative">
                                {member?.profileImageUrl ? (
                                    <img src={member.profileImageUrl} className="w-full h-full object-cover" alt="Profile" />
                                ) : (
                                    <LucideUserCircle className="w-10 h-10 text-white/15" />
                                )}
                        ...
                        <div className="flex-1 min-w-0">
                            {isEditingName ? (
                                <div className="flex items-center gap-2 mb-1.5">
                                    <input
                                        ...
                                        className="bg-transparent border-b border-white/25 text-[19px] font-bold text-white w-full max-w-[150px] outline-none"
                                        autoFocus
                                        onKeyDown={(e) => {
                                ...
                                <div className="flex items-center gap-2 mb-1.5">
                                    <h2 className="text-[19px] font-bold text-white tracking-tight truncate">
                            ...
                            <div className="flex items-center gap-2">
                                <span className="text-[13px] font-medium text-white/50">일반 회원</span>
                                <span className="w-1 h-1 rounded-full bg-white/25" />
                                <span className="text-[13px] font-medium text-white/40">Lv.{Math.floor((member?.visitCount || 0) / 5) + 1}</span>
                            </div>
                        </div>
                    </div>
                        ...
                        >
                            <div className="flex items-center gap-3.5">
                                <div className={`w-11 h-11 rounded-tile flex items-center justify-center ${item.danger ? 'bg-red-500/10' : 'bg-surface-2'}`}>
                                    <item.icon className={`w-5 h-5 ${item.danger ? 'text-red-400' : 'text-white/55'}`} />
                                </div>
                                <div className="text-left">
                                    <p className={`text-[15px] font-semibold leading-tight ${item.danger ? 'text-red-400' : 'text-white'}`}>{item.label}</p>
                                    <p className="text-[12.5px] text-white/40 font-medium mt-0.5">{item.desc}</p>
                                </div>
                            </div>
                            <LucideChevronRight className={cn("w-5 h-5", item.danger ? 'text-red-400/30' : 'text-white/45')} />
                        </motion.button>
                    ))}
                </div>
            </div>
            ...
            <motion.button
                ...
                className="relative z-10 w-full p-6 rounded-card overflow-hidden border border-brand/20 bg-gradient-to-br from-brand/[0.10] to-transparent mb-10 text-left"
            >
                <div className="flex items-center justify-between gap-4">
                    <div className="min-w-0">
                        <span className="rk-chip bg-brand/[0.12] text-brand mb-3">파트너 프로그램</span>
                        <h3 className="text-[20px] font-bold text-white mt-2 leading-tight">사장님이신가요?</h3>
                        <p className="text-[13px] font-medium text-white/50 mt-1">우리 매장도 랭큐로 스마트하게 운영하기</p>
                    </div>
            ...
            <div className="relative z-10 mb-12">
                ...
                <div className="flex flex-col gap-2.5">
                    <a href="https://www.tohk.co.kr" target="_blank" rel="noopener noreferrer" className="block rk-card p-5 hover:bg-surface-2 transition-colors group">
                        ...
                        <h3 className="text-[16px] font-semibold text-white leading-tight mt-1.5">어젯밤 그 꿈, 혹시 태몽? 흉몽?</h3>
                        <p className="text-[12.5px] text-white/40 font-medium mt-1 group-hover:text-purple-300/80 transition-colors">광고 없는 100% 무료 운세 바로가기</p>
                    </a>
                    ...
                    <a href="https://www.polli.co.kr" target="_blank" rel="noopener noreferrer" className="block rk-card p-5 hover:bg-surface-2 transition-colors group">
                        <span className="rk-chip bg-cyan-500/12 text-cyan-300 mb-2.5">대국민 투표 · 퀴즈</span>
                        <h3 className="text-[16px] font-semibold text-white leading-tight mt-1.5">짜장 vs 짬뽕, 당신의 선택은?</h3>
                        <p className="text-[12.5px] text-white/40 font-medium mt-1 group-hover:text-cyan-300/80 transition-colors">폴리에서 내 취향 증명하고 결과 보기</p>
                    </a>
                </div>
            </div>
