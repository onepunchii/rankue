            ...
            <div className="max-w-7xl mx-auto">
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-10">
                    <div>
                        ...
                        <p className="text-black/60 mt-2 font-medium">당구장 운영 및 회원 통합 관리 시스템</p>
                    </div>
                    <div className="flex items-center gap-3">
                        <Button
                            variant="outline"
                            onClick={handleDownloadExcel}
                            className="bg-transparent border-surface-line text-black/60 hover:text-[rgba(0,0,0,0.87)] hover:bg-surface-3"
                        >
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
                        ...
                        <CardContent className="p-6">
                            <div className="flex items-center justify-between mb-4">
                                <div className="p-3 bg-brand/10 rounded-2xl">
                                    <LucideUsers className="w-6 h-6 text-brand" />
                                </div>
                                <span className="text-xs font-semibold text-brand bg-brand/10 px-2 py-1 rounded-lg">실시간</span>
                            </div>
                            <p className="text-black/60 text-sm font-semibold">전체 회원 수</p>
                            <h3 className="text-4xl font-bold tabular-nums mt-1">{stats?.totalMembers || 0}<span className="text-lg text-black/40 font-normal ml-1">명</span></h3>
                        </CardContent>
                    </Card>
                    ...
                    <Card className="rk-card">
                        <CardContent className="p-6">
                            ...
                            <p className="text-black/60 text-sm font-semibold">금일 방문자</p>
                            <div className="flex items-baseline gap-2 mt-1">
                                <h3 className="text-4xl font-bold tabular-nums">{stats?.visitsToday || 0}</h3>
                                <span className="text-lg text-black/40 font-normal">명</span>
                                <span className="text-[12px] text-black/40 ml-auto tabular-nums">어제: {stats?.visitsYesterday || 0}명</span>
                            </div>
                        </CardContent>
                    </Card>
                    ...
                    <Card className="rk-card">
                        <CardContent className="p-6">
                            ...
                            <p className="text-black/60 text-sm font-semibold">금일 신규 가입</p>
                            <h3 className="text-4xl font-bold tabular-nums mt-1">{stats?.newToday || 0}<span className="text-lg text-black/40 font-normal ml-1">명</span></h3>
                        </CardContent>
                    </Card>
                </div>
                ...
                <Tabs defaultValue="members" className="w-full">
                    <TabsList className="bg-surface-1 p-1 mb-6 rounded-tile">
                    ...
                    <TabsContent value="members" className="space-y-6">
                        <div className="flex flex-col md:flex-row gap-4 items-center justify-between rk-card p-4">
                            <div className="relative w-full md:w-96">
                                <LucideSearch className="absolute left-4 top-1/2 -translate-y-1/2 text-black/40 w-5 h-5" />
                                <Input
                            ...
                            <div className="flex items-center gap-4">
                                <p className="text-sm text-black/60 font-medium tabular-nums">{selectedIds.length}명 선택됨</p>
                                <Button onClick={selectAll} variant="ghost" className="text-black/60 hover:text-[rgba(0,0,0,0.87)]">전체 선택</Button>
                            </div>
                        </div>
                                    ...
                                    <thead>
                                        <tr className="bg-surface-2 border-b border-surface-line">
                                            <th className="p-5 w-16">
                                                <div className="w-6 h-6 border-2 border-black/20 rounded cursor-pointer" onClick={selectAll} />
                                            </th>
                                            <th className="p-5 font-semibold text-black/60">성함</th>
                                            <th className="p-5 font-semibold text-black/60">연락처</th>
                                            <th className="p-5 font-semibold text-black/60 text-center">핸디 (3/4)</th>
                                            <th className="p-5 font-semibold text-black/60 text-center">방문수</th>
                                            <th className="p-5 font-semibold text-black/60">가입일</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        {filteredMembers.map((member) => (
                                            ...
                                            >
                                                <td className="p-5">
                                                    <div className={`w-6 h-6 border-2 rounded flex items-center justify-center transition-all ${selectedIds.includes(member.id) ? "bg-brand border-brand" : "border-black/20"}`}>
                                                ...
                                                <td className="p-5">
                                                    <div className="flex items-center gap-3">
                                                        ...
                                                        <div>
                                                            ...
                                                            <span className="text-xs text-brand font-semibold">일반</span>
                                                        </div>
                                                    </div>
                                                </td>
                                                <td className="p-5 font-mono text-black/60">{member.phone}</td>
                                                ...
                                                <td className="p-5 text-black/40 text-sm tabular-nums">{new Date(member.createdAt).toLocaleDateString()}</td>
                                            </tr>
                                        ))}
                                    </tbody>
                                </table>
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                            <Card className="rk-card p-8">
                                ...
                                <div className="space-y-6">
                                    <div>
                                        <label className="text-sm font-semibold text-black/60 block mb-2">수신 대상 ({selectedIds.length}명)</label>
                                        <div className="bg-surface-2 rounded-2xl p-4 max-h-32 overflow-y-auto flex flex-wrap gap-2  text-black/60 text-sm">
                                    ...
                                    <div>
                                        <label className="text-sm font-semibold text-black/60 block mb-2">메시지 내용</label>
                                        <Textarea
                                        ...
                                        />
                                        <p className="text-right text-xs text-black/40 mt-2 font-mono tabular-nums">
                            ...
                            <div className="space-y-6">
                                <Card className="bg-brand/5 border-brand/20 border-dashed rounded-2xl p-8">
                                    ...
                                    <p className="text-black/60 leading-relaxed font-medium">
                                        ...
                                        <span className="text-[rgba(0,0,0,0.87)] font-bold ml-1">'(광고)' 문구 및 '무료수신거부' 번호</span>가 <br />
                                        자동으로 포함되어 발송됩니다.
                                    </p>
                                </Card>

                                <Card className="rk-card p-8">
                                    <h4 className="font-semibold text-black/60 mb-6">최근 발송 내역</h4>
                                    <div className="space-y-4">
                                        {[1, 2, 3].map(i => (
                                            <div key={i} className="flex items-center justify-between p-4 bg-surface-2 rounded-2xl ">
                                                <div>
                                                    <p className="font-bold">당구대 교체 완료 기념 이벤트...</p>
                                                    <span className="text-xs text-black/40 font-mono tabular-nums">2024.01.10 14:20</span>
                                                </div>
                                                <span className="px-3 py-1 bg-surface-3 rounded-full text-xs font-bold text-black/60 tabular-nums">201명 발송</span>
                                            </div>
                                        ))}
                                    </div>
                                </Card>
...
function GolfOrderTabContent() {
    ...
    if (isLoading) return <div className="text-center py-20 text-black/60">불러오는 중...</div>;
            ...
            <Card className="rk-card overflow-hidden">
                <div className="overflow-x-auto">
                    <table className="w-full text-left">
                        <thead>
                            <tr className="bg-surface-2 border-b border-surface-line">
                                <th className="p-5 font-semibold text-black/60">날짜</th>
                                <th className="p-5 font-semibold text-black/60">구분</th>
                                <th className="p-5 font-semibold text-black/60">회원권 / 구장</th>
                                <th className="p-5 font-semibold text-black/60 text-right">희망가격</th>
                                <th className="p-5 font-semibold text-black/60">연락처</th>
                                <th className="p-5 font-semibold text-black/60 text-center">상태</th>
                                <th className="p-5 font-semibold text-black/60 text-center">관리</th>
                            </tr>
                        </thead>
                        <tbody>
                            {sortedOrders.map((order) => (
                                <tr key={order.id} className="border-b border-surface-line hover:bg-black/[0.03] transition-colors">
                                    <td className="p-5 text-black/40 font-mono text-sm max-w-[120px]">
                                        <div className="font-bold text-[rgba(0,0,0,0.87)] tabular-nums">{new Date(order.createdAt).toLocaleDateString()}</div>
                                    ...
                                    <td className="p-5 text-right font-mono font-bold text-[rgba(0,0,0,0.87)] text-lg tabular-nums">
                                        {new Intl.NumberFormat('ko-KR').format(order.price)}원
                                    </td>
                                    <td className="p-5 font-mono text-brand font-bold tabular-nums">{order.contact}</td>
                                    <td className="p-5 text-center">
                                    ...
                                    <td className="p-5 text-center">
                                        <select
                                            className="bg-surface-2 rounded-lg px-3 py-2 text-sm outline-none focus:border-brand text-black/70 font-medium cursor-pointer hover:bg-surface-3 transition-colors"
                                            value={order.status}
