...
describe("요청 매핑", () => {
    it("toCreateMatchBody: 설정의 알려진 필드만(선수·배치 없음, 여분 필드 안 샘)", () => {
        ...
        expect(body).toEqual({
            ...
            aimAssist: true, fullPreview: false,
        });
...
describe("파생값", () => {
    ...
    it("matchConfig: HUD·파라미터용 설정, target 은 내 다마수", () => {
        ...
        expect(c).toEqual({ gameType: "3c", tableId: "DAEDAE", target: 20, rules: DEFAULT_3C_RULES, finishType: "none", inningCap: 0, cushionModel: "han2005", condition: 1, mode: "normal", matchPreview: "short" });
