...
## 적구 먼저 길 자리 (2026-09-08 오너: "왜 다 뱅크샷이야?")
- 실측(무작위 배치 4개, 0.1° × 세기 4 × 당점 7 전수 40만 회): 득점 길의 **85 % 가 뱅크 먼저, 15 % 가 적구 먼저**. 쿠션은 표적이 크고 적구는 작아서
  여유(오차 허용)로 줄 세우면 목록이 전부 뱅크가 된다(화면 다섯 중 58/60). 실제 경기는 적구 먼저가 대부분이라 그 길도 보여 줘야 한다.
- `BALL_FIRST_SLOTS = 2`: 정제(refine) 대상과 최종 후보 모두 적구 먼저 가족에 최소 두 자리(있을 때만). 맨 뒤 뱅크 하나를 내리고 넣는다.
  판정은 라벨과 같은 기준(큐 방향 + 스쿼트 → 첫 접촉이 공). 자리는 "확보분 + 나머지 점수순" 으로 만든다(맨 뒤를 빼고 넣기를 반복하면 방금 넣은 걸 다시 뺀다).
  정제 이웃은 첫 접촉 종류가 같을 때만 대표가 된다(얇은 적구 샷의 ±0.5° 이웃은 쿠션 먼저로 뒤집힌다). 결과: 화면 다섯 중 뱅크 58 → 36/60, 1등 여유 평균 29 → 38 %.
- 화면 쪽(bestPath.rankedPaths)에도 같은 자리 규칙이 있다 — 솔버가 자리를 주면 화면은 여유 순으로 세운다.
