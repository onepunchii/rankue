...
고침        간접 접촉 파울은 득점하지 못한 샷에만 적용한다(evaluate.ts 4c). 내 공이 상대공을 직접 맞힌 건 실제 규칙대로 언제나 파울.
           토글 설명도 정확히 바꿨다("두 적구를 다 맞힌 샷은 그대로 득점이에요"). 기본값은 여전히 꺼짐.
점검 도구    scripts/sim-e2e/score-audit.ts(감점 판정·방 규칙) · foul-replay.ts(그 샷을 재시뮬해 직접/간접을 가른다)
남은 것      기존 방에 저장된 규칙은 그대로다. 이미 깎인 점수는 되돌리지 않았다.
```
