...
오른쪽 바      길 1·2·3 — 우리 공 색(백구·노란 공·빨간 공) 위에 번호, 캡션은 성공률. 누르면 그 길을 골라 경로를 그린다.
              그 아래 무작위 배치 · 다이아몬드(기본 켜짐) · 시점 · 나가기. 길 찾기(돋보기) 버튼은 없다 — 길이 없을 때 카드의 버튼 하나로 충분(오너).
샷            오른쪽 아래 샷 버튼이 "고른 길 그대로 치기"다(적용 + 재생). 길이 아직 없으면 먼저 길을 찾는다.
아래 카드      solver/BestPathCard — 두 줄만: 조준 문구 + 성공률 / 당점 그림 · 세기 막대 · 쿠션. 버튼은 없다(카드가 높으면 테이블 아래가 잘린다).
머리글         "길 찾기" 는 누를 수 없는 이름표(TopBar drillNameStatic), 상태 칩("연습")은 감춘다(hideStatus).
