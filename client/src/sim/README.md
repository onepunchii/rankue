...
- `simReducer.ts`: `simReducer(state, action)`, `createSimStore()`, `INITIAL_STATE`, `paramsFromConfig`, `clampPower/clampElevation/clampSpin`, `defaultPhi/thicknessPhi/objectTargetFor`, 상수 `V0_DEFAULT=2.5, V0_MIN=0.2, V0_MAX=7.5(화면 상한) · V0_LEGACY_MAX=9(엔진·옛 기록), THETA_MAX=20°, MAX_RETRIES=3`. 세기 눈금은 `controlsMath.POWER_GAMMA=1.45` 으로 휘어 있다(퍼센트 → 속도 = min + range·x^1.6).
...
나가기      왼쪽 뒤로 화살표를 없애고 오른쪽 끝 닫기(CloseIcon, 44 px)가 그 일을 한다 — 멈춤 아이콘은 시계가 멈추는 줄 알게 해서 바꿨다(2026-09-26). 왼쪽이 비어야 이름이 들어간다.
...
```
예외          랭킹 페이지에서만 그라데이션·광택·진한 색을 쓴다(오너 결정). index.css 의 .rank-arcade 스코프 안에만 두고 다른 화면에 복사하지 않는다.
배경          검은 바탕(온라인게임 구역과 같다) 위에 보라 판 — 참고 이미지도 어두운 바탕이라 판이 도드라진다. 흰 바탕으로 바꾸려면 페이지 래퍼의 sim-dark 만 빼면 된다.
필터 칩       고른 칩은 금색(--arc-frame) + 진한 글씨, 나머지는 흰 반투명 — 검은 배경에서 안 보이던 문제를 고쳤다(실측).
rank/RankBoard.tsx  리본 제목 + 보라 판. 줄 색: 1 금 · 2 하늘 · 3 산호 · 4위부터 흰 반투명(내려갈수록 옅게).
              한 줄 = 순위 · 아바타(이름 첫 글자) · 국기 + 티어 + 전적 · 레이팅 코인. 내 줄은 흰 테두리 + "나".
필터          칩 넷(3쿠션·대대 / 3쿠션·중대 / 4구·대대 / 4구·중대)을 **종목 칩 둘 + 테이블 토글 하나**로 줄였다(2026-09-09 오너).
              종목은 공 그림(GameBalls — 3쿠션 3개 · 4구 4개)에 등재 인원을 함께 적는다. 대대·중대는 오른쪽 작은 토글.
              **레이팅은 합치지 않는다** — 테이블마다 난이도가 달라 사람마다 두 개의 독립된 점수가 있다. 화면만 줄인 것이다.
첫 화면        내가 가장 많이 친 조합 → 없으면 등재자가 가장 많은 조합으로 연다(rankLadder 가 combos 로 인원·내 대전 수를 함께 준다).
              링크로 조합을 지정했거나 사용자가 한 번 고르면 그 뒤엔 자동으로 바꾸지 않는다.
rank/flag.ts  국가 코드 → 국기. lib/flag.ts 와 같은 규칙이지만 여기 둔다 — 시뮬 테스트는 "@" 별칭을 안 타서 별칭 import 가 있으면 파일이 안 열린다.
내 카드        같은 보라 판 위에 레이팅·순위·티어 사다리·국기. 4위 이하도 같은 리더보드 안에 있다.
```

### 이모지 인사 — 실패 사례 기록
```
증상          버튼은 보이는데 아무 일도 일어나지 않았다(상대 화면에도 안 뜸).
원인          matchApi 객체에 sendEmoji 구현이 빠져 있었다(인터페이스에만 선언). 컨트롤러가 `!this.matchApi.sendEmoji` 로 조용히 "failed" 를 돌려줬다.
고침          구현 추가 + 실패 시 토스트(sim.emoji.failed) — 조용한 실패를 다시는 만들지 않는다.
표시 위치      처음엔 이름표 위 절대 위치 말풍선이었는데 375 px 화면 오른쪽 밖으로 잘렸다(실측 x=345). 보내기 버튼 옆 인라인 칩으로 바꿨다.
확인          scripts/sim-e2e/emoji-ui.ts — 브라우저 둘(방장·손님)로 보내고 받는 것까지 본다.
```

### 아케이드 배색을 어디까지 쓰나 (2026-09-09 오너와 합의한 경계선)
```
쓰는 곳        경쟁·성취를 보여 주는 화면 — 랭킹(전면) · 이번 주 드릴(전면) · 멀티방(판과 버튼만).
안 쓰는 곳     숫자를 읽는 화면(대시보드·기록)과 조작하는 화면(설정·대전 중)에는 보라 판·리본을 얹지 않는다 — 그래프 선과 숫자가 배경과 싸운다.
              대신 대시보드는 **같은 검정 구역(sim-dark) + 강조색**만 가져왔다(2026-09-09 오너 "디자인이 아쉽다"):
              개인 최고 기록(최고 에버·하이런)은 금색, 연패는 흐리게, 최근 10경기는 한 줄 막대(승 초록 · 패 회색).
              동그라미 열 개는 좁은 화면에서 두 줄로 접혀 흐름이 안 보였고, 높이를 다르게 했더니 진 경기가 사라진 것처럼 보였다 — 높이는 같게 색으로만 가른다.
드릴           문제마다 별(성공 = 금색 채움 · 실패 = 빈 별 · 미시도 = 흐린 별), 위에 별 진행 + 주차, 아래 주간 순위판(1·2·3 색).
              성공한 문제 줄은 금색 알약이라 한눈에 몇 개 깼는지 보인다.
멀티방         판(arc-board)과 리본, 방 만들기·참가 버튼만 금색. 방 줄은 정보가 많아 담백하게 둔다.
껍데기          드릴·멀티방 화면도 sim-dark 검은 배경(판이 도드라진다).
확인           scratchpad/shots/drills.mjs · scripts/sim-e2e/rooms-view.ts(임시 방 3개 만들고 캡처 뒤 정리)
```

### 4구 점수 사고 (2026-09-09 오너 신고: "상대공과 안 맞았는데 점수가 내려간다")
```
증상        4구 대전에서 -10 이 붙었다. 재시뮬로 보니 내 공은 상대공을 건드리지 않았다 — 적구가 굴러가 상대공을 밀었을 뿐이다.
           더 나쁜 건 그 샷이 두 적구를 다 맞힌 득점 샷이었다는 것(+10 이 -10 이 되어 20점이 뒤집혔다).
원인        방 규칙 passiveOpponentContactIsFoul(간접 접촉 파울)이 켜져 있었고, 판정에서 파울 검사가 득점 검사보다 먼저였다.
1차 고침    간접 접촉 파울을 득점하지 못한 샷에만 적용했다.
최종        오너가 "그런 룰이 원래 있냐"고 물었고 — 없다. 어느 당구장에도 없는 규칙이라 아예 없앴다(같은 날).
           판정은 이제 `foul = walk.directOpponentContact` 한 줄이다. 내 공이 상대공을 직접 맞힌 것만 파울.
           방 만들기 토글·규칙 배지·문구 키(optPassiveFoul, rule4cPassive)를 전부 지웠다.
           Rules.passiveOpponentContactIsFoul 필드는 옛 방 호환용으로만 남아 있다 — 켜져 있어도 판정·배지 모두 무시한다.
           새 방은 항상 false 를 보낸다. 되살릴 일 없으니 필드까지 지우려면 저장된 방을 마이그레이션해야 한다.
점검 도구    scripts/sim-e2e/score-audit.ts(감점 판정·방 규칙) · foul-replay.ts(그 샷을 재시뮬해 직접/간접을 가른다)
다마수       200다마 = 캐롬 20개(×10 관행). pointUnit 10 은 이 관행과 맞다 — 80다마면 8개. 오너 확인 완료.
남은 것      이미 깎인 점수는 되돌리지 않았다(끝난 경기 결과가 바뀐다).
```
