...
```
예외          랭킹 페이지에서만 그라데이션·광택·진한 색 알약을 쓴다(오너 결정). 스타일은 index.css 의 .rank-arcade 스코프 안에만 두고
              다른 화면으로 복사하지 않는다 — 앱의 나머지는 기존 규칙(그라데이션·발광·이모지 금지) 그대로다.
rank/RankBoard.tsx  보라 판 + 금색 테두리 + 리본 제목("리더보드"). 줄마다 색: 1 금 · 2 하늘 · 3 산호 · 4위부터 흰 반투명(순위가 내려갈수록 옅게).
              한 줄 = 순위 · 아바타(이름 첫 글자, 프로필 사진은 랭킹 API 에 없다) · 이름 + 티어·국가·전적 · 레이팅 코인. 내 줄은 흰 테두리와 "나".
RankPage      내 카드에 티어 사다리(일곱 칸, 현재 금색) + "다이아까지 18점". 4위 이하도 같은 리더보드 안에 있다.
확인          scratchpad/shots/rank.mjs(실데이터) · rank-many.mjs(응답만 가짜로 바꿔 여러 명일 때 — 서버·DB 는 건드리지 않는다)
```
