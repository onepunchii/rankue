...
### 랭킹 화면 — 시상대와 티어 사다리 (2026-09-09 오너: "명예로움 + 재미")
```
rank/RankPodium.tsx   톱 3 는 목록이 아니라 시상대. 1위가 가운데·가장 높고 왕관(인라인 SVG), 2·3위가 양옆.
                      게임 리더보드의 느낌은 자리 높이와 금·은·동(--gold/--silver/--bronze 토큰)으로만 낸다 —
                      그라데이션·발광·이모지는 우리 규칙에서 쓰지 않는다. 내가 시상대에 있으면 이름이 brand 색.
RankPage              내 카드에 티어 사다리 추가: 일곱 칸 중 지금 티어는 금색, 지나온 티어는 진한 먹, 남은 티어는 옅게.
                      아래 줄에 "플래티넘까지 38점" — 다음 목표가 보이는 게 재미다. 4위부터는 기존 목록.
```
