...
오른쪽 바      길 1·2·3(성공률이 캡션, 누르면 그 길을 골라 경로를 그린다) + 다시 찾기 · 무작위 배치 · 다이아몬드(길 찾기에선 기본 켜짐) · 시점 · 나가기.
아래 카드      solver/BestPathCard — 고른 길 한 장: 조준 문구 · 성공률(오차 허용) · 당점 그림(공 위의 점) · 세기 막대 · 쿠션 수.
              버튼 둘: "이대로 쳐 보기"(적용 + 재생을 한 번에) · "이 배치로 연습"(같은 배치를 연습 모드로 — 직접 치고 싶을 때의 통로).
