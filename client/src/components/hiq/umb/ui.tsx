    ...
    return (
        <section className="min-w-0">
            <div className="flex items-center justify-between gap-2 mb-3">
                ...
                {meta && <div className="min-w-0 truncate text-[12px] font-semibold text-black/45">{meta}</div>}
            </div>
            {desc && <p className="text-[11.5px] font-medium text-black/40 leading-snug -mt-1.5 mb-3">{desc}</p>}
            {children}
        </section>
    );
}
...
export function Tile({ emoji, value, label, accent }: { emoji: string; value: string; label: string; accent?: boolean }) {
    return (
        <div className="rounded-2xl bg-black/[0.03] px-2 py-3 text-center min-w-0">
            ...
            <div className="text-[10.5px] font-semibold text-black/45 mt-1.5 truncate">{label}</div>
        </div>
    );
}
...
export function Chip({ children, tone = "neutral", className }: { children: ReactNode; tone?: "neutral" | "gold" | "brand"; className?: string }) {
    return (
        <span className={cn(
            ...
            tone === "gold" ? "bg-[#F5B721]/15 text-[#8a6a0a]" : tone === "brand" ? "bg-brand/10 text-brand" : "bg-black/[0.04] text-ink-2",
            className,
        )}>{children}</span>
    );
}
...
export function List({ children, className }: { children: ReactNode; className?: string }) {
    return <div className={cn("rounded-2xl bg-black/[0.03] divide-y divide-black/[0.06] overflow-hidden", className)}>{children}</div>;
}
