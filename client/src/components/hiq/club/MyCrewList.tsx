...
export const MyCrewList = memo(({ currentSport }: MyCrewListProps) => {
    ...
    if (isLoading) {
        return <div className="h-40 bg-white/5 rounded-card animate-pulse" />;
    }
        ...
        return (
            <div className="rk-card flex flex-col items-center justify-center py-12 px-6 text-center">
                <div className="w-14 h-14 rounded-full bg-white/5 flex items-center justify-center mb-4">
                    <LucideTent className="w-7 h-7 text-white/40" />
                </div>
                <p className="text-white font-semibold text-[16px] mb-1">크루가 없으신가요?</p>
                <p className="text-white/45 text-[13px] font-medium mb-5">새로운 크루를 만들어 시작해보세요</p>
                <Button
