...
export function getRivalryLabel(wins: number, losses: number): RivalryLabel | null {
    ...
    if (diff <= -3) return { textKey: "h2h.labelPredator", className: "bg-red-500/10 text-red-500" };
    if (diff >= 3) return { textKey: "h2h.labelDominant", className: "bg-brand/12 text-brand" };
    if (decided >= 4 && Math.abs(diff) <= 1) return { textKey: "h2h.labelEven", className: "bg-black/[0.06] text-black/55" };
    return null;
}
...
const DOT_TITLE: Record<MatchResult, string> = { W: "formBadges.win", L: "formBadges.loss", D: "formBadges.draw" };
        ...
        <div className="flex items-center gap-1.5">
            {Array.from({ length: 5 }).map((_, i) => {
                ...
                return (
                    <span
                        key={i}
                        title={r ? t(DOT_TITLE[r]) : t("h2h.noRecord")}
    ...
    return (
        <div className="rk-card p-5">
            ...
            <div className="flex items-center justify-between mb-4">
                <span className="text-[12px] font-semibold text-black/45">{t("h2h.title")}</span>
                <div className="flex items-center gap-2">
                    ...
                    <span className="text-[12px] font-semibold text-black/55 tabular-nums">
                        {t("h2h.winRate").replace("{n}", String(winRate))}
                    </span>
                </div>
            </div>
            ...
            <div className="flex items-end gap-3">
                <div className="flex-1 min-w-0 text-right">
                    <div className="text-[13px] font-semibold text-black/55 mb-1">{t("h2h.me")}</div>
            ...
            <div className="mt-2 flex items-center justify-between text-[12px] font-medium text-black/45 tabular-nums">
                <span>{wins}{t("formBadges.win")}</span>
                {draws > 0 && <span>{draws}{t("formBadges.draw")}</span>}
                <span>{losses}{t("formBadges.loss")}</span>
            </div>

            {/* 최근 5경기 */}
            <div className="mt-5 flex items-center justify-between">
                <span className="text-[12px] font-medium text-black/55">
                    {t("h2h.recentFive")} <span className="text-black/35">· {t("h2h.newestFirst")}</span>
                </span>
                ...
                <>
                    ...
                    <div className="flex items-center">
                        <div className="flex-1 text-center">
                            <div className="text-[11px] font-medium text-black/45 mb-1">{t("h2h.myAvg")}</div>
                        ...
                        <div className="flex-1 text-center">
                            <div className="text-[11px] font-medium text-black/45 mb-1 truncate px-1">
                                {t("h2h.opponentAvg").replace("{name}", opponentName)}
                            </div>
                    ...
                    {avgTypeLabel && (
                        <div className="mt-2.5 text-center text-[11px] font-medium text-black/35">
                            {t("h2h.avgBasis").replace("{type}", avgTypeLabel)}
                        </div>
                    )}
                </>
            )}
