    ...
    return (
        <div className="space-y-4 mb-10">
            <header className="mb-1">
                <h2 className="text-[19px] font-bold tracking-tight text-ink-1">{t("trendChartCard.title")}</h2>
                <p className="text-black/55 text-[13px] mt-1 flex items-center gap-1.5 font-medium">
                    <LucideInfo className="w-3.5 h-3.5" /> {t("trendChartCard.subtitle")}
                </p>
            </header>
            ...
            <div className="bg-black/[0.04] p-1 rounded-2xl flex mb-6 relative h-11">
                ...
                <button onClick={() => setActiveTrendTab('3C')} className={cn("flex-1 rounded-xl text-[14px] font-semibold relative z-10 transition-colors bg-transparent border-none outline-none", activeTrendTab === '3C' ? "text-brand" : "text-black/45")}>{t("trendChartCard.tab3c")}</button>
                <button onClick={() => setActiveTrendTab('4B')} className={cn("flex-1 rounded-xl text-[14px] font-semibold relative z-10 transition-colors bg-transparent border-none outline-none", activeTrendTab === '4B' ? "text-brand" : "text-black/45")}>{t("trendChartCard.tab4b")}</button>
            </div>

            {activeTrendTab === '3C' ? (
                <SmoothTrendChart
                    key="3c-chart"
                    title={t("trendChartCard.chart3cTitle")}
                    subTitle={t("trendChartCard.chart3cSubtitle")}
                ...
                <SmoothTrendChart
                    key="4c-chart"
                    title={t("trendChartCard.chart4cTitle")}
                    subTitle={t("trendChartCard.chart4cSubtitle")}
