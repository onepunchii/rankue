...
// NOTE: SplitText is a premium GSAP plugin. If this import fails, 
// ensuring the user has the correct license/registry setup is required.
// For now, we follow the user's provided code structure.
// If 'gsap/SplitText' is not available, this needs to be adjusting or a trial file used.
// We'll trust the user has it or we might need to fallback.
// In standard npm 'gsap', SplitText is NOT included.
// Often it is imported from 'gsap/all' or 'gsap/dist/SplitText' if available.
// We will try to be safe but the user provided specific import.
// Let's assume standard import for now and see if we can maybe Mock it or if the user has it.
// UPDATE: To ensure this doesn't crash the app if missing, I will add a check or try-catch logic isn't possible at import level.
// I will proceed with the code provided.
...
const Shuffle: React.FC<ShuffleProps> = ({
    ...
    // We define refs for timeline and split instance
    const splitRef = useRef<any>(null); // Typing as any to avoid TS errors if types missing
    ...
    useGSAP(
        () => {
            ...
            const build = () => {
                ...
                // Ensure SplitText exists
                if (typeof GSAPSplitText === 'undefined') {
                    console.warn("GSAP SplitText plugin is missing. Text shuffle animation will be skipped.");
                    return;
                }
                ...
                chars.forEach(ch => {
                    ...
                    // if (!w) return; // Sometimes width is 0 if hidden, skipping check might be safer or not
